...
@router.get('/')
async def get_payments(current_user_role:Role_Pydantic=Depends(get_current_user_role)):
...
@router.post('/')
async def post_payment(payment: PaymentIn_Pydantic, current_user_role:Role_Pydantic=Depends(get_current_user_role)):
    if current_user_role.role != 'Admin':
        raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail=f'You are not authorised to use this resource')
...
@router.get('/{id}')
async def get_single_payment(id:int, current_user_role:Role_Pydantic=Depends(get_current_user_role)):
    if current_user_role.role != 'Admin':
        raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail=f'You are not authorised to use this resource')
...
@router.put('/{id}')
async def update_payment(payment: PaymentIn_Pydantic, current_user_role:Role_Pydantic=Depends(get_current_user_role)):
    if current_user_role.role != 'Admin':
        raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail=f'You are not authorised to use this resource')
